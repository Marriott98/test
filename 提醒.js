...
// var name_list = ["Alice Zhang", "陈靖", "黄佳璐"];
var name_list = ["奇188-han", "奇188-264", "奇159-my", "奇155-w", "奇159-3323", "奇159-3539", "奇155-gzhu"];
var chk_col = "#fff2fef2";
// var chk_col = "#ff196f55";
// var chk_col = "#ff12811f";
const cut_line = "==============================================================";
...
function remind_energy() {
    ...
    sleep(1000);
    for (var x = 0; x < 233; x++) {
        swipe(540, 2000, 540, 1200, 100);
        sleep(1000);
        if (text("没有更多了").exists()) break;
    }
    for (var x = 0; x < name_list.length; x++) {
        if (num > 5) break;
        text(name_list[x]).depth(15).findOne(2000).parent().parent().click();
        text("点击展开好友动态").depth(14).waitFor();
        toast_log("😇进入 " + name_list[x] + " 森林😇", 1);
        sleep(1000);
        var chk = findColorEquals(captureScreen(), chk_col, 100, 400, 700, 200);
        if (chk) {
            toast_log("😇准备点击😇", 1);
            click(chk.x, chk.y + 50);
            sleep(500);
            click(500, 310);
            chk = text("帮好友复活能量").depth(14).findOne(3000);
            if (chk) {
                if (textContains("确认发送").depth(14).findOne(1000).click()) {
                    toast_log("😇复活成功😇", 1);
                    num += 1;
                } else toast_log("👿发送失败👿", 1);
            } else toast_log("👿点击失败👿", 1);
        }
        sleep(500);
        backto("总排行榜", "总排行榜");
    }
