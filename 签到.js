...
function iqiyi() {
    launchApp("爱奇艺");
    toast_log("🎬🎬进入爱奇艺🎬🎬");
    var cancelUp = threads.start(function() {
        id("cancel_btn").waitFor();
        id("cancel_btn").click();
        toast_log("🎬🎬取消升级🎬🎬");
    })
    id("navi0").waitFor();
    click("我知道了");
    text("我的").depth(dep).findOne(1000).parent().click();
    text("观看历史").depth(9).waitFor();
    sleep(1500);
    var chk = text("黄金VIP会员").depth(10).findOne(2000);
    if (chk) {
        sleep(500);
        click(chk.bounds().centerX(), chk.bounds().centerY());
        sleep(1000);
        click(chk.bounds().centerX(), chk.bounds().centerY());
    } else {
        toast_log("🎬🎬【签到失败】🎬🎬");
    ...
    text("我的等级").depth(11).waitFor();
    chk = text("去签到").depth(12).findOne(1000);
    if (chk) {
        click(chk.bounds().centerX(), chk.bounds().centerY());
        text("签到成功").depth(8).waitFor();
        toast_log("🎬🎬签到成功🎬🎬");
        click("立即领取");
        sleep(1000);
        className("android.view.ViewGroup").clickable(true).depth(8).findOne().click();
    } else {
        let rw = text("更多任务").findOne(2000);
        if (rw) {
            toast_log("🎬🎬更多任务🎬🎬", 1);
            click(rw.bounds().centerX(), rw.bounds().centerY());
            className("android.widget.TextView").text("任务说明").waitFor();
            if (text("签到记录").findOne(2000)) {
                toast_log("🎬🎬已签到🎬🎬");
            } else {
                toast_log("🎬🎬坐和放宽🌚🎬🎬");
            }
        } else toast_log("🎬🎬进入更多任务失败🎬🎬");
    }
    cancelUp.interrupt();
    goback(2);
}
...
function dongcheng() {
    launchApp("东呈会");
    toast_log("🏨🏨进入东呈会🏨🏨");
    id("text").className("android.widget.TextView").text("首页").waitFor();
    sleep(1000);
    id("text").className("android.widget.TextView").text("首页").findOne().click();
    sleep(500);
    id("content").className("android.widget.TextView").text("签到/任务").findOne().parent().parent().click();
    className("android.widget.TextView").text("补签日历").waitFor();
    sleep(1000);
