...
function record_score(mess) {
    ...
    full = className("android.view.View").untilFind();
    for (var x = 0; x < full.length; x++) {
        if (tx = full[x].getText()) {
            if (i == 0 && num < 4) {
                if (num == 0 && mess == '总排行榜') {} else {
                    tmp[i] = num;
                    num++;
                    i++;
                }
            ...
            if ((i > 2) && (tx.search("\\d+t") != -1 || tx.search("\\d+kg") != -1 || tx.search("\\d+g") != -1)) {
                if (x < 4) {
                    rec.push(x);
                }
                rec.push(tmp);
                tmp = [];
                i = 0;
                if (num == 0 && mess == '总排行榜') num++;
            }
        }
    }
    if (rec.length) {
        tx = "";
        for (var x = 0; x < rec.length; x++) tx = tx + '[' + rec[x] + ']\n';
        files.append("./log/log_mi_" + mess + ".txt", '🏆🏆🏆【' + get_time() + '】【' + mess + '】🏆🏆🏆\n' + tx + "=================================================\n", encoding = 'utf-8');
    }
}
